...
export default class NewsCrd extends Component {
  ...
  // }
  render() {
    let { urlToImage, description, url, key } = this.props;
    return (
      <div>
        <div className="card p-4 m-4" style={{ width: "18rem" }}>
          ...
          <div className="card-body">
            <h5 className="card-title">
              {description ? description.slice(0, 40) : "No Des Found"}...
            </h5>
            <p className="card-text">
              {/* Some quick example text to build on the card title and make
                    up the bulk of the card's content. */}
            </p>
            <a href={url ? url : "No Url Found"} className="btn btn-primary" target="_blank">
              Go somewhere
            </a>
          </div>
        </div>
